...
# External SDKs that must stay behind repo/ adapters. The video pipeline adds
# AI providers to the original boto3 rule.
EXTERNAL_SDKS = ("boto3", "botocore", "openai", "anthropic", "faster_whisper")


def test_external_sdks_only_in_repo():
    """Verify external SDKs (boto3 + AI providers) are only imported in app/repo/."""
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
            continue
