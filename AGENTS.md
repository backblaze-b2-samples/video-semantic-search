...
This app started from the Backblaze B2 full-stack starter foundation (Next.js + FastAPI). The pieces below are load-bearing — keep them.

**Reusable B2 scaffolding (keep as-is)**
- **UI kit / design system.** `apps/web/src/components/ui/` (shadcn primitives), the design tokens in `apps/web/src/app/globals.css`, and the `/design` reference page. Build screens with these primitives; never edit the generated `components/ui/` files directly — restyle through tokens in `globals.css`.
- **File Explorer (full-bucket browse).** `/files`, `apps/web/src/app/files/`, `apps/web/src/components/files/`, and its sidebar entry. This is the bucket explorer — keep it.
- **Upload (generic).** `/upload`, `apps/web/src/app/upload/`, `apps/web/src/components/upload/`, and its sidebar entry — for small assets proxied through the API. Large video uses the multipart **Ingest** flow instead, not this.

**This app's core**
- **Library** (`/library`, `apps/web/src/components/library/`) — sample-scoped explorer of ingested videos with pipeline status, plus the multipart Ingest dialog ("Add video").
- **Search** (`/search`, `apps/web/src/components/search/`) — natural-language question → timestamped clips.
- **Pipeline** — repo: `video_store.py` (B2 multipart + artifacts), `media.py` (ffmpeg), `transcription.py`, `embeddings.py`, `llm.py`; service: `ingest.py`, `search.py`, `videos.py`, `transcript.py`; runtime: `videos.py`, `search.py`.
- **Dashboard** (`/`, `apps/web/src/components/dashboard/`) — video metrics (videos indexed, minutes transcribed, chunks embedded) via the `useVideos` hook, plus a B2 activity chart. New aggregations flow through TanStack Query hooks in `apps/web/src/lib/queries.ts` — no bare `useEffect + fetch`.

**Why this contract exists**
- The UI kit, Files (bucket explorer), and Upload pages are the reusable B2-backed scaffolding. The video Library, Search, and pipeline are this app's reason to exist. Keep both.
...
- No external SDKs (`boto3`, `openai`, `anthropic`, `faster_whisper`) outside `repo/`
