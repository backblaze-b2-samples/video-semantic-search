...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-video-semantic-search)",
    );
  }
}

function checkFfmpeg() {
  // Warn (not fail): ffmpeg is only needed for the video ingest pipeline;
  // the generic upload/files features run fine without it.
  if (!tryExec("ffmpeg -version")) {
    warn(
      "ffmpeg not found — the video ingest pipeline can't extract audio without it",
