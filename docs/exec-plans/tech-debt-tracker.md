...
|---|---|---|---|---|
| Ingest pipeline implemented but not verified end-to-end against live providers | Whisper/embeddings/Claude code paths are unexercised (no keys at build time) | Run a real ingest with `OPENAI_API_KEY` + ffmpeg, confirm `transcript.json`/`embeddings.json` and a search round-trip; add a recorded/mocked integration test | High | Open |
